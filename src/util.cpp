...
const char * const BITCOIN_CONF_FILENAME = "genex.conf";
const char * const BITCOIN_PID_FILENAME = "genexd.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Genex
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Genex
    // Mac: ~/Library/Application Support/Genex
    // Unix: ~/.bitcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Genex";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Genex";
#else
    // Unix
    return pathRet / ".genex";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix1, const std::string& strPrefix2)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Genex Core") == std::string::npos) {
